...
class Watcher:
    def __init__(self, detector, index_path, benchmark_url_prefixes):
        ...
        self._hash_to_revision = {v: k for k, v in self._index_data['revision_to_hash'].items()}
        ...
        self._processed_benchmarks = self.process_benchmarks(self._index_data['benchmarks'])
    ...
    def process_benchmarks(self, benchmarks):
        ...
        for benchmark in tqdm(benchmarks):
            ...
            param_combos = [dict(zip(param_names, values)) for values in it.product(*params)]
    ...
    def identify_regressions(self, data):
        ...
        for (name, asv_params), data in data.items():
            ...
            if regression is not None:
                result[regression._bad_hash] = result.get(regression._bad_hash, []) + [regression]
        return result
    ...
    def generate_report(self, regressions: list[Regression]):
        ...
        print(
            'Subsequent benchmarks may have skipped some commits. See the link below to see which commits are'
            ' between the two benchmark runs where the regression was identified.\n'
        )

        key, data = regressions[0]._asv_name, regressions[0]._data
...
def make_param_string(param_names, param_combo):
    ...
    return '; '.join([f'{name}={value}' for name, value in zip(param_names, param_combo)])
