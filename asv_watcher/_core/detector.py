...
class RollingDetector(Detector):
    ...
    def detect_regression(self, data: pd.DataFrame):
        ...
        data['established_worst'] = (
            data.groupby(keys, as_index=False)['time'].rolling(self._window_size, center=True).max()[['time']]
        )
        data['established_best'] = (
            data.groupby(keys, as_index=False)['time'].rolling(self._window_size, center=True).min()[['time']]
        )
        data['established_worst_cummin'] = gb['established_worst'].cummin()
        data['established_best_cummin_rev'] = data['established_best'][::-1].groupby(keys).cummin()[::-1]

        mask = data['established_worst_cummin'] < tol * data['established_best_cummin_rev']
